...
% ========
shot = 204660;
constraints.t = [0 0.4 0.8 0.88]; % 0:0.2:0.9; % [0 0.4 0.6 0.8];   
constraints.n = length(constraints.t);

opts.plotit = 0;
coil_constraints = fetch_coilcurrents_nstxu(shot, constraints.t, opts);
...
init = fetch_coilcurrents_nstxu(shot, 0, opts);
ic0 = init.icx;
iv0 = init.ivx;
ip0 = init.ip;
x0 = [ic0; iv0; ip0];
...
figure
hold on
plot(t,xhat(circ.iicx,:), 'b')
for i = 1:length(coil_constraints.times)
  scatter(ones(circ.ncx,1) * coil_constraints.times(i), coil_constraints.icx(:,i), 'filled')
end
...
figure 
hold on
plot(t, xhat(circ.iipx,:), 'b')
plot(targs.t, targs.ip, '--r')
