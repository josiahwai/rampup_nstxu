...
clearvars -except targets tok_data_struct original_efit01_eqs debug
...
for i = 1:ntargs
  ...
  r = [targets.rcp(i,:)];
  z = [targets.zcp(i,:)];
  rz = solveTSP([r' z'], 0); % sort via traveling salesman
  rz = [rz; rz(1,:)];
  n = length(rz);
  ...
  eqs(i) = variables2struct(rbbbs, zbbbs, jphi, psizr_pla, pcurrt);
  ...
  if debug
    ...
    figure
    contourf(rg,zg,original_efit01_eqs.gdata(i).pcurrt)
    colorbar
    title('EFIT')
    ...
    psizr_pla2 = mpp * original_efit01_eqs.gdata(i).pcurrt(:);
    psizr_pla2 = reshape(psizr_pla2, nr, nz);
