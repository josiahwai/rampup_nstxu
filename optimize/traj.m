...
% clearvars -except targets tok_data_struct eqs
% 
...
r = [targets.rcp(i,:) targets.rbdef(i)];
z = [targets.zcp(i,:) targets.zbdef(i,:)];
rz = solveTSP([r' z'], false); % sort via traveling salesman
rz = [rz; rz(1,:)];
n = length(rz);
...
if debug
  ...
  figure
  contourf(rg,zg,pcurrt)
  colorbar
  title('Estimate')
  
  figure
  contourf(rg,zg,eqs.gdata(i).pcurrt)
  colorbar
  title('EFIT')
...
%%
% r = linspace(min(rbbbs), max(rbbbs));
% z = zc * ones(size(r));
% p1 = bicubicHermite(rg, zg, pcurrt, r, z);
% p2 = bicubicHermite(rg, zg, eqs.gdata(i).pcurrt, r, z);
% 
% 
% figure
% hold on
...
figure
contourf(rg, zg, psizr_pla, 30)
colorbar
axis equal

psizr_pla2 = mpp * eqs.gdata(i).pcurrt(:);
psizr_pla2 = reshape(psizr_pla, nr, nz);

figure
contourf(rg, zg, psizr_pla2, 30)
colorbar
axis equal
...
% contourf(rg, zg, pcurrt)
% colorbar
% set(gcf, 'Position', [440 369 334 429])
% axis equal
% 
% figure
% contourf(rg, zg, eqs.gdata(i).pcurrt)
% colorbar
% set(gcf, 'Position', [778 371 302 426])
% axis equal
% 
% 
% figure
