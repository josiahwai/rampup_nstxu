...
typedef struct {unsigned long w1; unsigned long w2;} quadword;
#define CONST
typedef char mxChar;
#ifdef HOW_TO_COMPILE
cc/g_float/inc=matlab:[extern.include]/defi=V4 mdsipmex.c
my_cmexg :== @my_cmexg.com
my_cmexg mdsipmex sock serv servip
#endif
...
#define DEFAULTSERVER "atlas.gat.com"
#endif
#define SERVERTEST    "::"
#define PORTTEST      ":"
#define STRLEN 128
...
/*       plhs[0] = mxCreateDoubleMatrix(0,0,mxREAL);*/
	 ...
	 cptr = ans.ptr;
	 ans.ndims = 2;
	 matdum[0] = 1; matdum[1] = ans.length;
	 for (i = 0; i < len; i++) {
...
/* output vector */
      ...
      plhs[0] = mxCreateDoubleMatrix(0,0,
				     ans.dtype == DTYPE_COMPLEX ? mxCOMPLEX :mxREAL);
#else
...
/* Calculate number of elements */
   ...
   printf("V4 [%d] replacing type [%d]\n",
	  i,mxIsChar(Tarray));
#else
   printf("[%d] replacing type [%d], ndims [%d]",
	  i,mxIsChar(Tarray),ndim);
   if(ndim >1)
...
/* find conversion */
   if(     !strcmp(temp,"DTYPE_USHORT"))  conv = DTYPE_USHORT;
   else if(!strcmp(temp,"DTYPE_ULONG"))   conv = DTYPE_ULONG;
   else if(!strcmp(temp,"DTYPE_CHAR"))    conv = DTYPE_CHAR;
   else if(!strcmp(temp,"DTYPE_SHORT"))   conv = DTYPE_SHORT;
   else if(!strcmp(temp,"DTYPE_LONG"))    conv = DTYPE_LONG;
   else if(!strcmp(temp,"DTYPE_FLOAT"))   conv = DTYPE_FLOAT;
/*   else if(!strcmp(temp,"DTYPE_DOUBLE"))  conv = DTYPE_DOUBLE;*/
   else if(!strcmp(temp,"DTYPE_DOUBLE"))  conv = 0;   /* default from matlab */
   else mexErrMsgTxt("mdsput: Conversion request not found");
#endif				       /* V4 */
...
/* check if the server ip is however correct */
	    ...
	    if((hent == NULL) || strncmp(*hent->h_addr_list,(char *)servip,MIN(hent->h_length,NSERVERIP)))  {
...
#ifdef DEBUG
	    printf("IP [%03d][%03d][%03d][%03d]  [%d]\n",servip[0],servip[1],servip[2],servip[3],hent->h_length);
#endif
	 }
      }
   } else
...
elseif(nargin< 2 & ~isstr(server));shot=server;server='EFIT01';
...
%
% Adapted from mdsvalue Basil P. DUVAL, May 2000
