...
% drsep
if isfield(targets,'drsep')
  if isfield(weights,'drsep')
    weight = weights.drsep;
  else
    weight = 1;
  end
  
  % identify most important upper and lower nulls
  i = find(nulls.z < zmaxis);
  [~,j] = min(abs(nulls.psi(i) - psibry));
  idown = i(j);
  ...
  rx = nulls.r([idown iup]);
  zx = nulls.z([idown iup]);  
  psix = nulls.psi([idown iup]);
  
  % get derivatives
  [romp, i] = max(rbbbs);
  zomp = zbbbs(i);
  [~,psi_r_omp] = bicubicHermite(rg, zg, psizr, romp, zomp);
    
  for i = 1:size(dpsizrdx,2)    
    dpsizrdx_i = reshape(dpsizrdx(:,i), nz, nr);    
    dpsixdx(:,i) = bicubicHermite(rg, zg, dpsizrdx_i, rx, zx);    
    [~, dpsi_r_omp_dx(i)] = bicubicHermite(rg, zg, dpsizrdx_i, romp, zomp);
  end
  
  drsep = (psix(2) - psix(1)) / psi_r_omp;
  ddrsepdx = (dpsixdx(2,:) - dpsixdx(1,:)) / psi_r_omp - ...
    ((psix(2) - psix(1)) / (psi_r_omp^2) * dpsi_r_omp_dx);   
  
  iev = iev+1;	    
	ev(iev,1) = weight*(drsep - targets.drsep);
	devdx(iev,:) = weight*ddrsepdx;
	str = 'drsep';
	strev(iev,1:length(str)) = str;    
end
...
% Boundary Gaps
% Specify that a point (brygap.r, brygap.z) is exactly brygap.psi away from psibry
if isfield(targets,'brygap')
  for i = 1:length(targets.brygap.r)
    if isfield(weights,'brygap') & length(weights.brygap) >= i
      weight = weights.brygap(i);
    else
      weight = 1;
    end
    r = targets.brygap.r(i);
    z = targets.brygap.z(i);
    ...
    if ~isnan(r) & ~isnan(z) & ~isnan(weight)
      ...
      if kr < 2 | kr > nr-2 | kz < 2 | kz > nz-2
        ...
        psisep = wsep*psizr(iisep);
        dpsisepdx = wsep*dpsizrdx(iisep,:);
        ev(iev,1) = weight*(psisep - psibry - targets.brygap.psi(i));
        devdx(iev,:) = weight*(dpsisepdx - dpsibrydx);
        str = ['brygap' num2str(i)];
        strev(iev,1:length(str)) = str;
      end
    end
  end
...
% this solves norm(grad(psi(targets.rgrad,targets.zgrad.z))) = targets.grad

if isfield(targets,'rgrad') & isfield(targets,'zgrad') & isfield(targets,'grad')
  for i = 1:length(targets.rgrad)
    if isfield(weights,'grad') & numel(weights.grad) >= i
      weight = weights.grad(i);
    else
      weight = 1;
    end
    r = targets.rgrad(i);
    ...
    if ~isnan(r) & ~isnan(z) & ~isnan(weight)
      ...
      if kr < 2 | kr > nr-2 | kz < 2 | kz > nz-2
        outofrange = true;
        if iteration_counter == 0
          disp(['warning in gsdesign: target rgrad = ' num2str(r), ...
            ', zgrad = ' num2str(z) ' is out of range, and will be ignored'])
        end
      else
        iix = kz+nz*(kr-1)+neighbors';
        wr0 = [1 tr tr^2 tr^3]*mx;
...
%         iev = iev+1;
%         ev(iev,1) = weight * (sqrt(psixr^2 + psixz^2) - targets.grad);
%         devdx(iev,:) = weight * 1/sqrt(psixr^2 + psixz^2) * ...
%           (psixr * dpsixrdx + psixz * dpsixzdx);
%         str = ['\partial\psi_gradpt' num2str(i) '/\partialRZ'];
%         strev(iev,1:length(str)) = str;
        
        iev = iev+1;
        ev(iev,1) = weight * (psixr - targets.grad(1));
        devdx(iev,:) = weight * dpsixrdx;
        str = ['\partial\psi_gradpt' num2str(i) '/\partialR'];
        strev(iev,1:length(str)) = str;
        
        iev = iev+1;
        ev(iev,1) = weight * (psixz - targets.grad(2));
        devdx(iev,:) = weight * dpsixzdx;
        str = ['\partial\psi_gradpt' num2str(i) '/\partialZ'];
        strev(iev,1:length(str)) = str;
      end
    end
  end
...
% Josiah Wai - 3/12/2021
% Specify a linear combination constraint on ic, of the form A*ic = b where
% A and b are supplied by spec.targets.ic_comb.A and spec.targets.ic_comb.b
% This can be used to e.g. design a change such that net zero Ip is induced
if isfield(targets,'ic_comb')
  for i = 1:length(targets.ic_comb.b)
    if isfield(weights,'ic_comb') & length(weights.ic_comb) >= i
      weight = weights.ic_comb(i);
    else
      weight = 1;
    end
    if ~isnan(targets.ic_comb.b(i)) & ~isnan(weight)
      iev = iev+1;            
      ev(iev,1) = weight*(targets.ic_comb.A * ic - targets.ic_comb.b);            
      dicdx = [eye(nc) zeros(nc, nx-nc)];
      devdx(iev,:) = weight * targets.ic_comb.A * dicdx;      
      str = ['ic linear comb' num2str(i)];
      strev(iev,1:length(str)) = str;      
    end
  end
end
...
if isfield(targets,'pres')
  for i = 1:length(targets.pres)
    weight = weights.pres(i);    
    iev = iev+1;
    ev(iev,1) = weight*(pres(i) - targets.pres(i));
    devdx(iev,indsp) = weight*( c0(i,:) + c1(i,:)*psibar(i) + c2(i,:)*psibar(i)^2 + c3(i,:)*psibar(i)^3);        
    str = ['pres' num2str(i)];
    strev(iev,1:length(str)) = str;
  end
end

if isfield(targets,'fpol')
  for i = 1:length(targets.fpol)
    weight = weights.fpol(i);    
    iev = iev+1;
    ev(iev,1) = weight*(fpol(i) - targets.fpol(i));
    devdx(iev,indsf) = weight * sign(rzero*bzero) * 1/sqrt(2 * fpol(i)) * ...
...
% Josiah Wai 3/5/2021
% Specify in flux space, that the target point is located close to 
% (within dpsi) of the boundary flux, psibry:  
% psi(limits.brygap.r,limits.brygap.z) - psibry < limits.brygap.dpsi_max
% psi(limits.brygap.r,limits.brygap.z) - psibry > limits.brygap.dpsi_min
if isfield(limits,'brygap')
  for i = 1:length(limits.brygap.r)
    r = limits.brygap.r(i);    
    z = limits.brygap.z(i);
    ir = (r-rg(1))/dr+1;
    iz = (z-zg(1))/dz+1;
    ...
    Ai(end+1,:) = (dpsisepdx - dpsibrydx) * dxdxcirc(:,iopen);
    bi(end+1,1) = limits.brygap.dpsi_max(i) - (psisep - psibry);    
    Ai(end+1,:) = -(dpsisepdx - dpsibrydx) * dxdxcirc(:,iopen);
    bi(end+1,1) = -limits.brygap.dpsi_min(i) + (psisep - psibry);        
  end
end
