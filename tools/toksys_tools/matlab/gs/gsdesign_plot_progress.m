...
if ~exist('ha','var')                       | ...
   ...
   ~ishandle(ha.figure)                     | ...
   ~all(ishandle(ha.hs))                    | ...
   iteration_counter == 1 & fig < 0 & ~done | ...
   fig > 0 & ha.figure ~= fig
  ...
  ha.fs = 0.8./[60 60 60 100 60 60];  % fs1/window width in pixels
  ...
  hold(ha.hs(2),'on')
  ha.profiles.pprime = plot(ha.hs(2),psibar,psibar,'blue','LineWidth',3);
  ha.profiles.ffprim = plot(ha.hs(2),psibar,psibar,'red', 'LineWidth',3);
  ...
  ha.tokamak.extra.x = plot(nan,nan,'cx');
  ...
  ha.h1(end+1) = title(upper(tokamak));
...
if newhandles | iteration_counter == 1
...
elseif exist('iteration_counter','var')
  str = ['Iteration ' num2str(iteration_counter)];
...
if ishandle(ha.profiles.pprime)
  set(ha.profiles.pprime,'XData',psibar)
  set(ha.profiles.pprime,'YData',dumnr)
end
dumnr = ffprim/mu0/rmaxis;
if min(dumnr)*max(dumnr) < 0 % min must be negative and max positive
...
if ishandle(ha.profiles.ffprim)
  set(ha.profiles.ffprim,'XData',psibar)
...
for i = 1:nulls.count
  if nulls.r(i) == rbdef & nulls.z(i) == zbdef
    ix = i; % Will be the first one almost always, if diverted plasma
  end
end
nxpt = min(nulls.count,nxptmax); % Number of rx, zx values in the list
nrow = 6+2*nxpt; % Total number of rows in the list
set(ha.scalars.cpasma.name,'Position', [0.1,0.985-1/nrow*0.9])
set(ha.scalars.cpasma.value,'Position', [0.4,0.990-1/nrow*0.9])
set(ha.scalars.cpasma.value,'String', sprintf('%8d',round(cpasma)))
set(ha.scalars.li.name,'Position', [0.1,0.985-2/nrow*0.9])
set(ha.scalars.li.value,'Position', [0.4,0.990-2/nrow*0.9])
set(ha.scalars.li.value,'String', sprintf('%7.5f',li))
set(ha.scalars.betap.name,'Position', [0.1,0.985-3/nrow*0.9])
set(ha.scalars.betap.value,'Position', [0.4,0.990-3/nrow*0.9])
set(ha.scalars.betap.value,'String', sprintf('%7.5f',betap))
set(ha.scalars.betan.name,'Position', [0.1,0.985-4/nrow*0.9])
set(ha.scalars.betan.value,'Position', [0.4,0.990-4/nrow*0.9])
set(ha.scalars.betan.value,'String', sprintf('%7.5f',betan))
set(ha.scalars.psimag.name,'Position', [0.1,0.985-5/nrow*0.9])
set(ha.scalars.psimag.value,'Position', [0.4,0.990-5/nrow*0.9])
set(ha.scalars.psimag.value,'String', sprintf('%7.5f',psimag))
set(ha.scalars.psibry.name,'Position', [0.1,0.985-6/nrow*0.9])
set(ha.scalars.psibry.value,'Position', [0.4,0.990-6/nrow*0.9])
set(ha.scalars.psibry.value,'String', sprintf('%7.5f',psibry))
for i = 1:nxptmax
  if i <= nxpt
    set(ha.scalars.rx.name(i),'String', ['R_x_' num2str(i)])
    set(ha.scalars.zx.name(i),'String', ['Z_x_' num2str(i)])
    set(ha.scalars.rx.name(i),'Position', [0.1,0.985-(5+2*i)/nrow*0.9])
    set(ha.scalars.zx.name(i),'Position', [0.1,0.985-(6+2*i)/nrow*0.9])
    set(ha.scalars.rx.value(i),'String', sprintf('%7.5f',nulls.r(i)))
    set(ha.scalars.zx.value(i),'String', sprintf('%7.5f',nulls.z(i)))
    set(ha.scalars.rx.value(i),'Position', [0.4,0.990-(5+2*i)/nrow*0.9])
    set(ha.scalars.zx.value(i),'Position', [0.4,0.990-(6+2*i)/nrow*0.9])
    if i == ix
...
if 0
subplot(ha.hs(4))
for i = 1:length(wrongway.p)
  text(wrongway.r(i),wrongway.z(i),num2str(wrongway.p(i)),'fontsize',18)
end
end
if ishandle(ha.tokamak.patch)
  set(ha.tokamak.patch,'XData',rbbbs(1:nbbbs))
  set(ha.tokamak.patch,'YData',zbbbs(1:nbbbs))
end
if plasma
  psia = psimag;
  psib = psibry;
...
for i = 1:nflux-1
  if exist('nhires','var') & nhires > 1
    ...
    ccc = contourc(rg,zg,1-(psizr-psia)/(psib-psia),i/nflux+[0 0]);
  end
  j = 1;
  n = size(ccc,2);
  while j < n
...
if plasma & nSOL > 0
  if rbbbso > 0
    rsol = rbbbso + plot_settings.SOL.d*(1:plot_settings.SOL.n);
    psisol = gs_interp2(rg,zg,psizr,rsol,zbbbso);
    for i = 1:nSOL
      if exist('nhires','var') & nhires > 1
	ccc = contourc(rghr,zghr,1-(px-psia)/(psib-psia), ...
          1-(psisol(i)-psia)/(psib-psia)+[0 0]);
      else
	ccc = contourc(rg,zg,1-(psizr-psia)/(psib-psia), ...
          1-(psisol(i)-psia)/(psib-psia)+[0 0]);
      end
      j = 1;
      n = size(ccc,2);
...
for i = nxptmax:-1:1
  if ishandle(ha.tokamak.x(i))
    if i <= nxpt & i ~= ix
      set(ha.tokamak.x(i),'XData',nulls.r(i))
      set(ha.tokamak.x(i),'YData',nulls.z(i))
    else
...
if ishandle(ha.tokamak.limits.x)
  set(ha.tokamak.extra.x,'XData',nan)
  set(ha.tokamak.extra.x,'YData',nan)
end
if exist('limits','var')
  ...
  if isfield(limits,'rx') & isfield(limits,'zx')
    if ishandle(ha.tokamak.limits.x)
      xpolys.r = limits.rx';
      xpolys.z = limits.zx';
      set(ha.tokamak.limits.x,'XData',xpolys.r(:))
      set(ha.tokamak.limits.x,'YData',xpolys.z(:))
    end
    if showallxpoints
      rxs = nulls_xlim.r(1:nulls_xlim.count);
      zxs = nulls_xlim.z(1:nulls_xlim.count);
      set(ha.tokamak.extra.x,'XData',rxs)
      set(ha.tokamak.extra.x,'YData',zxs)
    end
  end
end
if ishandle(ha.tokamak.bbbs)
  set(ha.tokamak.bbbs,'XData',rbbbs(1:nbbbs))
  set(ha.tokamak.bbbs,'YData',zbbbs(1:nbbbs))
end
if all(ishandle(ha.tokamak.bdef))
  set(ha.tokamak.bdef,'XData',rbdef)
  set(ha.tokamak.bdef,'YData',zbdef)
end
...
% UPDATE FLUX ERROR
dumzr = psizr_err/(psibry-psimag);
if ~all(ishandle(ha.fluxerror)) | numel(ha.fluxerror) ~= nr
...
axis(ha.hs(6),[0.5 nz+0.5 min(dumzr(:))-1e-16 max(dumzr(:))+1e-16])
