...
% Josiah Wai 1/4/2021
% In flux space, the target point should be located close to (within dpsi) 
% of the boundary flux, psibry:  
% abs( psi(limits.psisep.r,limits.psisep.z) - psibry) < limits.psisep.dpsi
if isfield(limits,'psisep')
  for i = 1:length(limits.psisep.r)
    r = limits.psisep.r(i);    
    z = limits.psisep.z(i);
    ir = (r-rg(1))/dr+1;
    iz = (z-zg(1))/dz+1;
    ...
    bi(end+1,1) = limits.psisep.dpsi - (psisep - psibry);    
    Ai(end+1,:) = -(dpsisepdx - dpsibrydx) * dxdxcirc(:,iopen);
    bi(end+1,1) = limits.psisep.dpsi + (psisep - psibry);        
  end
end
