function [xsdot, y] = gsevolve(xs, u, init, config)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%  WRITTEN BY:  Anders Welander  ON	10/20/13
%
...
persistent Amat Bmat psibarc ncont npola dvrescFdx dvrescFdetac eta_vs_rhot
...
persistent xpix zpix nxpix nypix fpix Pcg ilimgg ETAC
...
% FAST LOOP
if nargin == 2 & ~ischar(xs) & ~isempty(Amat) & ~converge    
  gs_update_fast % Use linear response
  if equilibrium_update_is_complete
    gs_xsdot
    % PLOT
    if (plot_counter <= length(plot_times) && time >= plot_times(plot_counter)) || ...
...
if ischar(xs)
  if nargin == 1
    if exist(xs,'var')
      xsdot = eval(xs);
    elseif strcmp(xs,'eq') % Return a structure eq
      gs_create_struct_eq
      xsdot = eval(xs);
    elseif strcmp(upper(xs),'FUN2WS') % All persistent to caller's ws
      vars = who;
      for i = 1:size(vars,1)
	assignin('caller',char(vars(i)),eval(char(vars(i))))
      end
    elseif strcmp(upper(xs),'WS2FUN') % From caller's ws to persistent
      vars = who;
      for i = 1:size(vars,1)
	if evalin('caller',['exist(''' char(vars(i)) ''',''var'')'])
      ...
      xsdot = [xs ' is not a persistent variable in gsevolve'];
    end
    y = [];
    return
  elseif nargin == 2
    if exist(xs,'var')
      eval([xs ' = u;']);
      xsdot = u;
    elseif strcmp(upper(xs),'HELP')
      gshelp(u,'gsevolve')
    else
      disp([xs ' is not a persistent variable in gsevolve']);
      xsdot = [xs ' is not a persistent variable in gsevolve'];
    end
    if nargout == 0
      clear xsdot
...
if time > 4.0
%  save(['gsev' num2str(round(1e6*time))])
end


% UPDATING EQUILIBRIUM (process input xs to make output y)
if converge
...
% DYNAMICS (process input u to make output xsdot)
if new_response_was_calculated
  gs_dynamics % Find Amat, Bmat with dynamics for system plasma+coils
end
if isempty(xs) % Return xs0 (from gs_dynamics) in the variable xsdot
  xsdot = xs0;
else
  gs_xsdot % Find xsdot for existing Amat, Bmat
end

% PLOT
if ~isempty(xs) && ~isempty(u)
  if new_response_was_calculated & plot_if_new_response
    gs_rtplot
  elseif (plot_counter <= length(plot_times) && time >= plot_times(plot_counter)) || ...
         (dtplot > 0 && time >= time_for_next_plot)
    gs_rtplot_prepare
    gs_rtplot    
    if dtplot > 0 && time >= time_for_next_plot
