%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  USAGE:   gsdesign_demo_iter
%
%  PURPOSE: Design equilibrium for ITER
%
%  INPUTS:  None (just run the code)
%
%  OUTPUTS: Plots and designed equilibrium
%
%  METHOD:  
	
%
%  WRITTEN BY:  Anders Welander  ON	4/17/14
%
...
iter.rx = +5.1171;
iter.zx = -3.4139;
ix = find(iter.zsep == iter.zx);


% Load ITER tokamak configuration
...
if exist('iter_obj_filename','var')
  try
    clear tok_data_struct
    load(iter_obj_filename)
    tok_data_struct.rg;
  catch
    error('iter_obj_filename does not hold name of a matfile containing tok_data_struct')
  end
else
  iter_obj_filename = [getenv('GATOOLS_ROOT'), ...
    '/tokamaks/iter/make/2010v3p3/iter_obj_2010v3p3_3365.mat'];
  if exist(iter_obj_filename,'file')
    load(iter_obj_filename)
  else
    iter_obj_filename = ...
      '/m/GAtools/tokamaks/iter/make/2010v3p3/iter_obj_2010v3p3_3365.mat';
    if exist(iter_obj_filename,'file')
      load(iter_obj_filename)
    else
      disp('Could not find iter objects. Please set the variable:')
      disp('iter_obj_filename')
      disp('to name of matfile containing tok_data_struct for iter')
    end
  end
end
...
spec.cccirc = [1 2 3 4 5 6 7 8 9 9 10 11 12 -12];

% Specify where the poloidal field should equal zero
spec.targets.rx = iter.rx;
spec.targets.zx = iter.zx;
spec.weights.x = 10000;

% Specify what point defines the boundary
spec.targets.rbdef = iter.rx;
spec.targets.zbdef = iter.zx;
spec.weights.bdef = 1;

% Target separatrix from ITER_data_2010-v3.3.xls
spec.targets.rsep = iter.rsep;
spec.targets.zsep = iter.zsep;

% Extra weight on divertor legs
idiv = find(spec.targets.zsep < iter.zx);
%spec.weights.sep(idiv) = 10;

% Specify plasma current, li, betap, boundary flux
spec.targets.cpasma = 15e6;
spec.weights.cpasma = 1e-2;
spec.targets.li = 1;
spec.weights.li = 1e4;
spec.targets.betap = 0.65;
spec.weights.betap = 1e4;
spec.targets.psibry = 0;
spec.weights.psibry = 1e4;

% Lock VS coils to 0
spec.locks.ic = nan(14,1);
spec.locks.ic(13:14) = 0;
