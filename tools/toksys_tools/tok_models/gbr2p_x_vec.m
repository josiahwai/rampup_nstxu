function [product]=gbr2p_x_vec(gbr2p,vec)
 %
%  SYNTAX:  product = gbr2p_x_vec(gbr2p,vec)
%
%  PURPOSE:  Compute product of matrix gbr2p and vector vec, where gbr2p is
%		stored in compressed format. Use mpp_x_vec for multiplication
%		with either mpp or gbz2p.
%
%  INPUT:
%	gbr2p = plasma grid to plasma grid mutuals in compressed format
%
%  OUTPUT:
%	product = product of matrix multiplication
%
%  SEE ALSO: get_plasma_greens, mpp_x_vec
...
if(length(vec)~=ngrids)
   disp('ERROR gbr2p_x_vec: input vector size must match gbr2p 1st dimension')
   product=[];
   return;
end
...
for j=1:ngrids		% index of output element of matrix multiply
   ...
   for m=1:nr
      ...
      for n1=1:nz
         n = z_idx-n1;
         k = k2+n1;
         gbrp(k) = sign(n+0.5)*gbr2p(k1+abs(n)+1,m);
      end
   end
