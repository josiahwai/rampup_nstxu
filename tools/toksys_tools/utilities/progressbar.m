...
% 2011-Feb-02   By Anders Welander: Added feature to write message in title
%
...
message = ''; % Message to include as initial text in title on progressbar
for j = 1:length(input) % A message meant for the title has to be preceeded by the string 'title:'
  if strfind(input{j},'title:')
    message = strrep(input{j},'title:','');
    input = input([1:j-1 j+1:end]);
    break
  end
end
...
set(progfig, 'Name', [message titlebarstr])
