...
set(gcf, 'Position', [531 410 379 650])
axis equal
...
% All of the (rbdef, zbdef) now form valid closed contours. Choose the most
% external contour
...
[~,~,~,rbbbs,zbbbs] = trace_contour(rg,zg,psizr,rbdef,zbdef,rmaxis,zmaxis,rlim,zlim,1,1);
rbbbs = rbbbs{1};
zbbbs = zbbbs{1};

% sort between upper and lower x-points
[~,i] = sort(abs(psix-psibry)); % distinguish importance by closeness to boundary
rx = rx(i);
zx = zx(i);
psix = psix(i); 
j = find(zx < zmaxis, 1);
...
% find major radius R0 and minor radius a
rmax = max(rbbbs);
rmin = min(rbbbs);
[zmax,i] = max(zbbbs);
rup = rbbbs(i);
