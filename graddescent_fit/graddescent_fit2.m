...
%   e_ip = e(end,:);
  ...
  voltage_scale(ifit) = voltage_scale(ifit) * min(max(1.5 .^ -e_avg(ifit), 0.8), 1.4);
  rcc(1)
%   R = [rcc; rvv];
%   rfactor = ones(size(R));
%   rfactor(ifit) = min(max(1.5 .^ e_avg(ifit), 0.8), 1.4);
%   R = R .* rfactor;
%   rcc = R(circ.iicx);
%   rvv = R(circ.iivx);
%   
...
subplot(414)
hold on
plot(fit_timebase, xfit(ifit, :), 'b')
plot(fit_timebase, xtarg(ifit,:), '--r')
ylabel(['Coil ' num2str(ifit)], 'fontsize', 14)
legend('Fit', 'Target', 'fontsize', 14)
