...
parameters = {'rc' rc; 'rv' rv; 'lc' lc; 'lv' lv; 'mcc_triu' mcc_triu; 'mvv_triu' mvv_triu; 'mcv' mcv}; 
odefun = 'coil_vessel_dynamics';

sys = idgrey(odefun, parameters, 'd', file_args, Ts, 'InputDelay', 3);

sys.Structure.Parameters(1).Free = false;  % rc
sys.Structure.Parameters(2).Free = true;   % rv
sys.Structure.Parameters(3).Free = false;  % lc
sys.Structure.Parameters(4).Free = true;   % lv
sys.Structure.Parameters(5).Free = false;  % mcc_triu
sys.Structure.Parameters(6).Free = false;  % mvv_triu
sys.Structure.Parameters(7).Free = false;  % mcv

f = 100;

sys.Structure.Parameters(1).Minimum = rc / f;  % rc
sys.Structure.Parameters(2).Minimum = rv / f;  % rv
sys.Structure.Parameters(3).Minimum = lc / f;  % lc
sys.Structure.Parameters(4).Minimum = lv / f;  % lv
sys.Structure.Parameters(5).Minimum = -1e-4;  % mcc_triu
sys.Structure.Parameters(6).Minimum = -1e-4;  % mvv_triu
sys.Structure.Parameters(7).Minimum = -1e-4;  % mcv

sys.Structure.Parameters(1).Maximum = rc * f;  % rc
sys.Structure.Parameters(2).Maximum = rv * f;  % rv
sys.Structure.Parameters(3).Maximum = lc * f;  % lc
sys.Structure.Parameters(4).Maximum = lv * f;  % lv
sys.Structure.Parameters(5).Maximum = max(mcc(:))*2;  % mcc_triu
sys.Structure.Parameters(6).Maximum = max(mvv(:))*2;  % mvv_triu
sys.Structure.Parameters(7).Maximum = max(mcv(:))*2;  % mcv
